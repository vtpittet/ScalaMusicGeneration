...
class SimpleGrammarFirstsTest extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("terminal firsts singleton") {
    SimpleWord(1).firsts shouldBe Set(1)
  }
  ...
  test("firsts of production after recursive rule") {
    ...
    lazy val r: SimpleProduction[Int] = (p1 ** p2 ** r ** 3, 1.0) || (e, 1.0)
  ...
  test("non-empty recursive production") {
    lazy val p: SimpleProduction[Int] = (1, 1.0) || (p, 1.0) || (2 ** 3, 1.0) || (e ** p, 0.0)
    p.firsts shouldBe Set(1, 2)
  }

  test("nullable recursive production") {
    lazy val p: SimpleProduction[Int] = (1, 1.0) || (p, 1.0) || (2 ** 3, 1.0) || (e ** p, 0.0) || ((e ** 4, 1.0) || (p ** p ** p, 1.0) || (e ** e, 1.0), 1.0)
