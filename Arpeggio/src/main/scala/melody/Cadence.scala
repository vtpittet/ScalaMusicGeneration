...
case class Cadence(tonic: Tone, arpeggio: Arpeggio, val tempo: Int) extends Melody {
  ...
  val tonicChord: Chord = MajorChord(tonic, arpeggio)
  val subdominantChord: Chord = MajorChord(subdominant, arpeggio)
  val dominantChord: Chord = MajorChord(dominant, arpeggio)
